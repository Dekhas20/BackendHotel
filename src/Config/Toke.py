...
def generar_token(user_token, pass_token):
    try:
        fecha_vencimiento = generar_fecha_vencimiento(minutos=120)["token"]
        ...
        return Out_response(200, "Token generado exitosamente", datos=token)
...
def verificar_token(token):
    try:
        ...
        return Error_response(err, "Firma de Token inválida", 401)
    except jwt.exceptions.DecodeError as err:
        return Error_response(err, "No se pudo decodificar el token", 401)
    except jwt.ExpiredSignatureError as err:
        return Error_response(err, "Token expirado", 401)
    except jwt.exceptions.InvalidAlgorithmError as err:
        return Error_response(err, "Algoritmo de Token inválido", 401)
    except jwt.exceptions.InvalidTokenError as err:
        return Error_response(err, "Token inválido", 401)
    except jwt.exceptions.InvalidKeyError as err:
        return Error_response(err, "LLave secreta de Token inválida", 401)
    except Exception as err:
        return Error_response(err, "Token Inválido", 401)
...
""",
        ...
        res = {
            "error": True,
            "mensaje": mensaje,
            "data": {"Code": codigo_error, "Mensaje Error": err.args[0]},
        }
